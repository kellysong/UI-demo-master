...
public class Utils {
     ...
     * @param <T>
     */
    public static <T> void openNewActivity(Context context,Class<T> cls) {
     ...
     * @param srvClass
     */
    public static void stopService(Context context, String serviceName, Class srvClass) {
        if (AppInfoUtils.isServiceRunning(context, serviceName)) {
            context.stopService(new Intent(context, srvClass));
        }else{
            Logger.w("没有这个服务："+serviceName);
        }
    }
